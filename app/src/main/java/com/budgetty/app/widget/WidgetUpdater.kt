 ...
 * transactions, or the currency change, every widget is re-rendered via Glance's `updateAll`.
 * Started once from [com.budgetty.app.BudgettyApplication]. (Rollover while the app is backgrounded
 * is handled by each receiver's periodic `APPWIDGET_UPDATE` tick.)
 */
class WidgetUpdater(
